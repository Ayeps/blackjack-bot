...
var RestApi = new JsonRestApi({port: PORT}, function (err) {
    ...
    if (!(is.nonEmptyStr(process.NODE_ENV) &&
        ...
        logger.warn('Adding debug routes');
        RestApi.addRoute('post', '/debugCredits', debugCredits);
        RestApi.addRoute('get', '/debugGetPlayer', debugGetPlayer);
        RestApi.addRoute('get', '/debugGameState', debugGameState);
    }
});
...
function logout(req, res, json) {
    var info;
    try {
        info = getInfoFromReq(req, res, json, 'logout', {player: true});
        if (info === false)
            return;
        assert.ok(info !== false);
        assert.ok(is.nonEmptyObj(info));
        assert.ok(is.nonEmptyObj(info.player));
        var tableId = info.player.tableId;
        if (tableId !== -1) {
            var hasId = Tables.all.hasOwnProperty(tableId);
            if (hasId) {
                var table = Tables.all[tableId];
...
function debugGetPlayer(req, res, json, qs) {
    var info;
    try {
        info = getInfoFromReq(req, res, qs, 'debug/credits', {table: true});
        if (info === false)
            return;
        assert.ok(info !== false);
        assert.ok(is.nonEmptyObj(info));
...
function debugGameState(req, res, json, qs) {
    var info;
    try {
        if (!is.obj(qs))
            throw new Error('Did not receive query string');
        var inspect = require('util').inspect;
        logger.error('>>>qs: ' + inspect(qs));
        var playerId = Math.floor(qs.playerId);
        if (is.nan(playerId)) {
            throw new Error('Received bad playerId: ' +
                JSON.stringify(qs.playerId));
        }
